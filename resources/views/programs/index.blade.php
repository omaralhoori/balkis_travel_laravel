        ...
        @foreach($programs as $program)
            <div class="program-card group bg-zinc-dark border border-white/5 rounded-2xl overflow-hidden card-shadow transition-all duration-500 hover:-translate-y-2" data-category="{{ $program->category }}" data-title="{{ strtolower($program->title) }}" data-description="{{ strtolower($program->description ?? '') }}">
                ...
                <div class="p-8">
                    ...
                    @endif
                    <a href="{{ route('programs.show', ['locale' => app()->getLocale(), 'slug' => $program->slug]) }}" class="w-full flex items-center justify-center gap-2 py-3 border border-primary/30 text-primary text-sm font-bold hover:bg-primary hover:text-zinc-dark transition-all group-hover:border-primary">
